import { encodeSecret, isAllowedStreamUrl } from "./obfuscate";

export function toSegmentProxyUrl(targetUrl: string): string {
  return `/api/segment?t=${encodeURIComponent(encodeSecret(targetUrl))}`;
}
  ...
  return manifest
    ...
    .map((line) => {
      ...
      try {
        ...
        return toSegmentProxyUrl(resolved);
      } catch {
        return line;
      }
    })
